...
class PublicInterface {
    ...
    async loadPredictionHistory() {
        try {
            ...
            // Load prediction history from the new endpoint
            const data = await PowerballUtils.apiRequest('/prediction-history-public?limit=25');
    ...
    displayPredictionHistory(data) {
        ...
        // Format dates using English locale
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                Recent ${data.count} predictions generated by our AI system
                            </p>
                        </div>
                        <div class="text-right">
                            <div class="text-2xl font-bold text-green-600 dark:text-green-400">${data.count}</div>
                ...
                <div class="overflow-x-auto max-h-80 overflow-y-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ${data.history.map((prediction, index) => `
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center space-x-2">
                                            ${prediction.numbers.map(num => `
                                                <span class="inline-flex items-center justify-center w-8 h-8 bg-white text-gray-900 rounded-full text-xs font-bold border border-gray-300 shadow-sm">${num}</span>
                                            `).join('')}
                                            <span class="text-red-500 text-sm font-bold mx-1">•</span>
                                            <span class="inline-flex items-center justify-center w-8 h-8 bg-red-600 text-white rounded-full text-xs font-bold shadow-md">${prediction.powerball}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="p-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
                    <p class="text-xs text-gray-600 dark:text-gray-400 text-center">
                        ...
                        Showing most recent predictions ordered by generation date (newest first)
                    </p>
                </div>
            </div>
        `;
