...
class ModelValidator:
    ...
    def validate_model_quality(self) -> Dict[str, any]:
        ...
        try:
            ...
            if model is None:
                ...
                validation_results['recommendations'].append('Model file not found or corrupted')
            ...
            validation_results['validation_metrics']['feature_compatibility'] = feature_compatibility
            
            if feature_compatibility['status'] == 'feature_mismatch':
                validation_results['overall_status'] = 'needs_retrain'
                validation_results['recommendations'].append('Feature shape mismatch detected - retrain required')
                return validation_results
            
            # 3. Verificar datos suficientes
            data_check = self._check_data_availability()
            validation_results['validation_metrics']['data_availability'] = data_check
            ...
            if not data_check['sufficient']:
                ...
                validation_results['recommendations'].append('Insufficient data for validation')
                return validation_results
            
            # 4. Evaluar performance reciente
            recent_performance = self._evaluate_recent_performance()
            validation_results['validation_metrics']['recent_performance'] = recent_performance
            
            # 5. Determinar estado general
            overall_status = self._determine_overall_status(validation_results['validation_metrics'])
            validation_results['overall_status'] = overall_status
            validation_results['recommendations'] = self._generate_recommendations(validation_results['validation_metrics'])
            
            logger.info(f"Model validation completed with status: {overall_status}")
            return validation_results
    ...
    def _check_feature_compatibility(self) -> Dict[str, any]:
        ...
        try:
            ...
            try:
                ...
                if model and hasattr(model, 'estimators_') and len(model.estimators_) > 0:
    ...
    def _check_data_availability(self) -> Dict[str, any]:
        ...
        try:
            ...
            recent_data = historical_data[historical_data['draw_date'] >= cutoff_date]
    ...
    def _evaluate_recent_performance(self) -> Dict[str, any]:
        ...
        try:
            ...
            performance_data = get_performance_analytics(days_back=self.validation_window_days)
    ...
    def _generate_recommendations(self, metrics: Dict) -> List[str]:
        ...
        if feature_compat.get('status') == 'feature_mismatch':
            recommendations.append("Feature shape mismatch detected - model retraining required")
        
        # Recomendaciones por performance
        perf_status = recent_perf.get('status', 'unknown')
        if perf_status == 'poor':
            recommendations.append("Poor recent performance - consider model retraining")
        elif perf_status == 'acceptable':
