...
async def get_smart_predictions(
    ...
    try:
        ...
        # Get recent predictions, filtering for those generated by the pipeline
        all_predictions = get_prediction_history(limit=limit * 2)  # Get more to filter

        # Filter for Smart AI predictions (those with high scores and recent timestamps)
        smart_predictions = []
        for pred in all_predictions:
            # Convert prediction to smart format
            smart_pred = {
                "rank": len(smart_predictions) + 1,
                "numbers": [pred["n1"], pred["n2"], pred["n3"], pred["n4"], pred["n5"]],
            ...
            if len(smart_predictions) >= limit:
                break
