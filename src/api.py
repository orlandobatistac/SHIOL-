...
async def get_smart_predictions(
    num_plays: int = Query(100, ge=50, le=500, description="Number of final predictions to return")
):
    """
    ...
    try:
        ...
        try:
            ...
            ensemble_predictions = predictor.predict_ensemble_syndicate(ensemble_count)
            for pred in ensemble_predictions:
                pred['ai_method'] = 'ensemble'
                pred['method_weight'] = 0.4
                pred['base_score'] = pred.get('ensemble_score', pred['score_total'])
            all_candidates.extend(ensemble_predictions)
            method_scores['ensemble'] = len(ensemble_predictions)
        ...
        try:
            ...
            deterministic_predictions = predictor.predict_diverse_plays(deterministic_count, save_to_log=False)
            for pred in deterministic_predictions:
                pred['ai_method'] = 'deterministic'
                pred['method_weight'] = 0.35
                pred['base_score'] = pred['score_total']
            all_candidates.extend(deterministic_predictions)
            method_scores['deterministic'] = len(deterministic_predictions)
        ...
        try:
            ...
            if adaptive_system:
                # Use adaptive system if available
        ...
        for candidate in all_candidates:
            ...
            smart_score = (
                ...
                diversity_bonus * 0.2 +            # Diversity factor (20%)
            ...
            smart_candidate = {
                ...
                'smart_ai_score': smart_score,
                'base_score': base_score,
                'ai_method': candidate.get('ai_method', 'unknown'),
        ...
        response = {
            ...
            "generation_method": "smart_ai_unified",
            "ai_analysis": {
                "candidates_evaluated": len(all_candidates),
                "methods_used": list(method_scores.keys()),
                "method_distribution": method_scores,
                "unique_combinations": len(final_predictions),
                "score_range": {
                    "highest": final_predictions[0]['smart_ai_score'] if final_predictions else 0,
                    "lowest": final_predictions[-1]['smart_ai_score'] if final_predictions else 0,
                    "average": sum(p['smart_ai_score'] for p in final_predictions) / len(final_predictions) if final_predictions else 0
                },
                "tier_distribution": {
                    "premium": len([p for p in final_predictions if p.get('tier') == 'Premium']),
                    "high": len([p for p in final_predictions if p.get('tier') == 'High']),
                    "medium": len([p for p in final_predictions if p.get('tier') == 'Medium']),
                    "standard": len([p for p in final_predictions if p.get('tier') == 'Standard'])
                }
            },
        ...
        logger.error(f"Error in Smart AI Prediction: {e}")
        raise HTTPException(status_code=500, detail="Smart AI Prediction failed")

def _calculate_smart_diversity_bonus(candidate: Dict, reference_pool: List[Dict]) -> float:
...
def _calculate_historical_performance_bonus(candidate: Dict) -> float:
    """Calculate bonus based on historical performance patterns."""
...
def _assign_smart_tier(smart_score: float) -> str:
    """Assign tier based on Smart AI Score."""
    if smart_score >= 0.8:
        return "Premium"
